...
const Addresses = () => {
  ...
  const handleDefaultAddressSetting = (address) => {
    if (!address.isDefault) {
      ...
      setAddresses((addresses) => {
        const updatedAddresses = [...addresses.map((elem) => {
    ...
    <div className="container">
      <div className="addresses">
        ...
        {loading ? (
          ...
          <>
            {addresses.length === 0 ? (
              <p className="addresses__no-data">You don't have any saved addresses yet</p>
            ) : (
              <div className="addresses__wrap">
                <p className="adresses__text">Choose default address</p>
                <div className="addresses__list">
                  {defaultAddress && [defaultAddress, ...addresses.filter((address) => address._id !== defaultAddress._id)].map((address, index) => (
                    <div
                      key={index}
